...
Actor* Actor_Spawn(ActorContext* actorCtx, PlayState* play, s16 actorId, f32 posX, f32 posY, f32 posZ, s16 rotX,
    ...
    // liboot: v0.1 spawned no other actors; v0.4 let the real EnElf (Navi)
    // through; v0.6 generalizes the cut into a registry: the zeroed shim
    // gActorOverlayTable itself. liboot_link_init registers a profile for
    // every vendored actor whose assets bound (EnElf, EnArrow, EnBom,
    // EnBoom, ArmsHook); everything else stays cut and callers already
    // handle a NULL return as "spawn failed". vramStart stays NULL so
    // registered ids take the "not an overlay" branch below.
    if (gActorOverlayTable[actorId].profile == NULL) {
        return NULL;
    }
