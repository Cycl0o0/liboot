...
static f32 Matrix_FixedToFloat(u16 integer, u16 fraction) {
    u32 bits = ((u32)integer << 16) | fraction;
    /* Avoid shifting into signed int (UB) and avoid relying on an
       implementation-defined u32 -> s32 conversion for negative values. */
    s64 value = (bits & 0x80000000u) ? (s64)bits - 0x100000000LL : (s64)bits;
    return (f32)value * (1.0f / 65536.0f);
}
...
void Matrix_MtxToMtxF(Mtx* src, MtxF* dest) {
    ...
    dest->xx = Matrix_FixedToFloat(m1[0], m2[0]);
    dest->yx = Matrix_FixedToFloat(m1[1], m2[1]);
    dest->zx = Matrix_FixedToFloat(m1[2], m2[2]);
    dest->wx = Matrix_FixedToFloat(m1[3], m2[3]);
    dest->xy = Matrix_FixedToFloat(m1[4], m2[4]);
    dest->yy = Matrix_FixedToFloat(m1[5], m2[5]);
    dest->zy = Matrix_FixedToFloat(m1[6], m2[6]);
    dest->wy = Matrix_FixedToFloat(m1[7], m2[7]);
    dest->xz = Matrix_FixedToFloat(m1[8], m2[8]);
    dest->yz = Matrix_FixedToFloat(m1[9], m2[9]);
    dest->zz = Matrix_FixedToFloat(m1[10], m2[10]);
    dest->wz = Matrix_FixedToFloat(m1[11], m2[11]);
    dest->xw = Matrix_FixedToFloat(m1[12], m2[12]);
    dest->yw = Matrix_FixedToFloat(m1[13], m2[13]);
    dest->zw = Matrix_FixedToFloat(m1[14], m2[14]);
    dest->ww = Matrix_FixedToFloat(m1[15], m2[15]);
}
