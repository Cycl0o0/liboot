...
s32 BgCheck_PolyIntersectsSubdivision(Vec3f* min, Vec3f* max, CollisionPoly* polyList, Vec3s* vtxList, s16 polyId) {
    ...
    Vec3f vb;
    Vec3f vc;
    /* liboot: these masks use all 32 bits after the vertex flags are packed
     * into the high byte.  Shift as u32 so host UBSan does not diagnose the
     * original signed `0x80 << 24`, while preserving the exact mask bits. */
    u32 flags[3];
